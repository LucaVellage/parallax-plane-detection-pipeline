...
def _get_hour_partitions(t_start, t_end):
    ...
    needed to cover the query window. Usually one, occasionally two.
    """
...
def _build_query(params, hour_partitions):
    """
    Builds a Trino SQL query for the given tile params and hour partitions.
...
def _query_trino(params, hour_partitions):
    """Executes the Trino query and returns a DataFrame."""
...
def run_adsb_query(params):
    """
    Returns raw ADS-B state vectors for a full Sentinel-2 tile.
    Loads from parquet cache if available, otherwise queries Trino.

    Args:
        params : dict from get_tile_query_params()
    Returns:
        DataFrame with columns: time, icao24, callsign, lat, lon,
                                baroaltitude, velocity, heading,
