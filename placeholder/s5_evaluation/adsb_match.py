...
adsb_match.py
─────────────
Spatially matches pipeline detections against ADS-B records
using haversine distance. Produces a unified GeoDataFrame with
one row per point (both sources) and match metadata.

Entry point: run_adsb_match(df_detections, df_adsb, params)
"""
...
def _haversine_m(lat1, lon1, lat2, lon2):
    """
    Vectorised haversine distance in metres between two points
    or between one point and an array of points.
    """
...
def _find_best_match(lat, lon, df_adsb, threshold_m):
    """
    For a single point (lat, lon), finds the nearest ADS-B record
    within threshold_m. Returns (distance_m, icao24, callsign) or
...
def _build_detection_rows(df_detections, df_adsb, threshold_m):
    """
    Builds one row per pipeline detection with match metadata.
    """
    rows = []
    for _, det in df_detections.iterrows():
        ...
        rows.append({
            ...
            'lat'              : det['lat'],
            'lon'              : det['lon'],
            # preserve useful detection columns
...
def _build_adsb_rows(df_adsb, matched_icao24s, params):
    """
    Builds one row per ADS-B record. Marks whether it was
    matched to a pipeline detection.
    """
    rows = []
...
def run_adsb_match(df_detections, df_adsb, params):
    """
    Matches pipeline detections against ADS-B records and returns
    a unified GeoDataFrame with one row per point (both sources).

    Args:
        df_detections : DataFrame from detections CSV
        df_adsb       : DataFrame from run_adsb_filter()
        params        : dict from get_tile_query_params()
        threshold_m   : match distance threshold in metres
    Returns:
        GeoDataFrame with columns:
            source, in_pipeline, in_adsb, in_visual,
            matched, match_distance_m, matched_icao24,
            matched_callsign, lat, lon, geometry
    """
