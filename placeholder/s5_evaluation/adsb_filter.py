"""
Filters raw ADS-B state vectors and extracts one representative
position per unique aircraft, corrected for Sentinel-2 scan timing.

Entry point: run_adsb_filter(df_raw, params)
"""
...
def _apply_filters(df, params):
    """
    Applies temporal, spatial, altitude and staleness filters
    to raw state vectors.
    """
    n0 = len(df)
    ...
    # Altitude filter, i.e. removes non-flying aircraft
    # using conservative threshold to work globally 
    df = df[
        (df['onground'] == False) &
...
def _select_best_ping(group, params):
    """
    For a single aircraft, selects the ping closest to the
    row-corrected acquisition time based on its latitude in the tile.
    """
    group     = group.sort_values('datetime').copy()
    lat_range = params['north'] - params['south']

    # Step 1 — find ping closest to tile centre time
...
def _get_unique_aircraft(df, params):
    """
    Returns one representative row per unique aircraft,
    position-corrected for Sentinel-2 scan timing.
    """
    records = [
...
def run_adsb_filter(df_raw, params):
    """
    Filters raw ADS-B state vectors and returns one representative
    position per unique aircraft, corrected for Sentinel-2 scan timing.

    Args:
        df_raw         : raw DataFrame from run_adsb_query()
        params         : dict from get_tile_query_params()
        min_altitude_m : minimum barometric altitude in metres
    Returns:
        DataFrame with one row per unique aircraft
    """
