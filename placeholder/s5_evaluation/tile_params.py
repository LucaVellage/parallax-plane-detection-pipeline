...
Entry point: get_cached_tile_params(tile, date)
"""
...
def _load_cache():
    """Loads the tile params cache from disk. Returns empty dict if not found."""
...
def _save_cache(cache):
    """Saves the tile params cache to disk."""
...
def _fetch_from_gee(tile, date):
    """
    Fetches Sentinel-2 acquisition parameters from GEE for a given
    tile and date. Returns a dict with all parameters serialised
    as strings/floats for JSON storage.
    """
    date_next = (
    ...
    # Bounding box
    # Try both with and without T prefix since filenames vary
    mask_candidates = [
...
def _deserialise_params(raw):
    """
    Converts a cached params dict (strings) back to Python objects
    with proper datetime types for use in the pipeline.
    """
    return {
        **raw,
...
def get_cached_tile_params(tile, date):
    """
    Returns acquisition parameters for a Sentinel-2 tile and date.
    Loads from local cache if available, otherwise fetches from GEE
    and saves to cache.

    Args:
        tile : str — MGRS tile ID e.g. '31UEU' or 'T31UEU'
        date : str — date string 'YYYY-MM-DD'
    Returns:
        dict with keys: tile, date, image_id, t_top, t_bottom,
