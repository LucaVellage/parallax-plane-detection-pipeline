...
metrics.py
──────────
Computes detection performance metrics by comparing pipeline results
against the ADS-B ground truth table.

PRIMARY GROUND TRUTH: ground_truth_adsb.csv, is_visible=True rows.
  — recall denominator = visible ADS-B records
  — fully independent of pipeline output, no circularity
...
Stratification: by background_type (int 0–5) across all scenes,
not by tile. A single scene may contribute to multiple background types.

Background encoding:
    0 = outside_bounds (excluded from stratified metrics)
    1 = water
    2 = vegetation
    3 = barren_land
    4 = urban
...
# Background display order for tables
BG_ORDER = [1, 2, 3, 4, 5]
...
def _load_ground_truth():
    """
    Loads both ground truth tables from disk.
    ...
    # Warn about any rows missing background_type (outside_bounds=0 is ok)
...
def _compute_counts(visual_gt, adsb_gt, bg_filter=None):
    ...
    # Apply background filter — exclude outside_bounds (0) always
    if bg_filter is not None:
        vg = vg[vg['background_type'] == bg_filter]
        ag = ag[ag['background_type'] == bg_filter]
    else:
        # Exclude outside_bounds from aggregate too
    ...
    # FP: pipeline detected, not visible (coded FPx by annotator)
    fp  = (pipeline_rows['is_visible'] == False).sum()

    # TP1: pipeline detected, visible, no ADS-B signal (untracked aircraft)
    tp1 = (
        (pipeline_rows['is_visible'] == True) &
...
def _r(v):
    """Round to 4dp or return None if NaN."""
...
def _build_metrics_table(visual_gt, adsb_gt):
    """
    Builds a metrics DataFrame with one row per background_type
    plus one aggregate row.
    """
    rows = []
    ...
    for bg_int in BG_ORDER:
        ...
        if counts['n_adsb_visible'] == 0 and counts['TP'] + counts['FP'] == 0:
            # No data for this background — skip
            continue
    ...
    # Aggregate (all backgrounds, excluding outside_bounds)
...
def _build_scene_metrics(visual_gt, adsb_gt):
    """
    Builds a per-scene metrics table (one row per image_id).
    Scene-level background is reported as the modal background_type
    across pipeline detections in that scene.
    """
    rows = []
    ...
    for image_id in all_image_ids:
        ...
        # Modal background_type (excluding outside_bounds=0)
        pip_scene = vg_scene[
...
def _build_detection_results(visual_gt, adsb_gt):
    """
    Builds detection_results — one row per pipeline detection.
    Classifies each as TP / FP / TP1 and carries background_type + note_code
    for FP diagnostic analysis.
    """
    pipeline = visual_gt[
...
def _build_gt_match_results(adsb_gt):
    """
    Builds gt_match_results — one row per ADS-B visible record.
    Labels each as TP (detected) or FN (missed).
    """
...
def _print_summary(metrics_df, scene_metrics_df):
    """Prints metrics table stratified by background_type."""
    print(f"\n{'─'*70}")
...
def run_metrics():
    """
    Computes detection performance metrics using visible ADS-B records
    as the primary ground truth denominator.

    Primary ground truth: ground_truth_adsb.csv (is_visible=True rows)
    Predictions: pipeline rows in ground_truth_visual.csv
    ...
    Metric definitions:
        ...
        Precision = TP / (TP + FP)     [TP1 excluded from denominator]
        Recall    = TP / (TP + FN)     [denominator = visible ADS-B]
        F1        = harmonic mean

    Stratification: by background_type int (1–5), excluding outside_bounds (0).

    Returns:
        metrics_df       : DataFrame — by-background metrics + overall
        scene_metrics_df : DataFrame — per-scene metrics
        det_results_df   : DataFrame — per-detection classification (TP/FP/TP1)
        gt_results_df    : DataFrame — per-ADS-B-visible match result (TP/FN)
    """
    print(f"\n{'─'*55}")
    ...
    metrics_df.to_csv(
        out_dir / 'metrics_by_background.csv', index=False
    )
    scene_metrics_df.to_csv(
        out_dir / 'metrics_by_scene.csv', index=False
    )
    det_results_df.to_csv(
        out_dir / 'detection_results.csv', index=False
    )
    gt_results_df.to_csv(
        out_dir / 'gt_match_results.csv', index=False
    )
